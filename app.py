...
{{
  ...
  "scene_description_zh": "用 1～3 句完整中文，把当前画面描述得尽量具体，必须同时包含：1）主要人物的身份 + 大致年龄 + 性别 + 外观特征（尤其是服装颜色/款式、是否有披风、头饰等）；2）此刻正在做的动作以及动作方向和速度感（例如：从画面右上高速滑向左下、抬手推开门、低头看手机），如果是运动镜头要写出身体前倾/后仰/起跳等姿态；3）所在的场景类型与空间结构（例如：临海山路、峡谷、城市高楼之间的空中轨道），分清前景/中景/背景里各有什么物体（悬崖、海面、树木、房屋、车流等），以及地形特征（蜿蜒、陡坡、悬空平台等）；4）镜头相对人物的位置和视角（例如：第一人称视角、紧贴背后的追随视角、肩膀后视角、从侧后方略仰拍、俯视俯冲等），以及镜头大致沿着什么方向运动。禁止只写“某人站在某处”或“人物在路上移动”这种模糊句子，必须写到让美术或 3D 场景师能按文字搭场景的程度；忽略画面中的 App UI 图标/文字/界面元素，不要把抖音/快手按钮写进描述。",
  ...
  "environment_detail_zh": "用 2～4 句中文，按照 前景 / 中景 / 背景 的层次，尽可能具体地描述场景环境。必须写出：1）空间类型（室内/室外、厨房/街道/仓库/办公室/宇宙飞船舱室等）；2）前景靠近镜头的物体和质感（例如桌面、栏杆、玻璃、光屏，写清颜色/材质/是否虚化）；3）中景主体周围的环境结构（墙面、柜子、机器、车辆、人群等）；4）背景中可识别的建筑/山体/城市天际线/窗外景色；5）地面和顶部的感觉（例如：水泥地、木地板、带油污的瓷砖地、裸露管线的天花板）。不要只写“在一个房间里”，必须写到能让美术师按文字搭景的程度。",

  "props_and_tech_detail_zh": "用 1～3 句中文，列出画面中最重要的 3～8 个道具/科技元素，并说明它们的外观、位置和状态。例如：“左前景是一个银色笔记本电脑，屏幕发出冷蓝色光；人物右手边有一台黑色咖啡机，机身有水渍反光；背景墙上挂着两幅抽象画；天空中有两架无人机从左向右飞过；画面右侧是一块半透明蓝色全息屏幕，悬浮在空中，显示数据图表”。不要只说“有很多科技感道具”，必须写出具体是什么、在哪儿、是什么质感。",

  "motion_detail_zh": "中文描述此时画面中可推断的动作趋势（人物/镜头/环境的动势，例如：主角向左冲刺，镜头反向跟拍；飞船从远处快速逼近；相机贴背视角随人物一起俯冲）。",
  ...
  "video_prompt_en": "一段英文视频提示词，适合给 SORA/VEO 使用。用 3-5 句描述：人物外观、当前动作、运镜方式（要说明是追随视角/第一人称/侧向跟拍等以及镜头移动方向）、环境地形（例如 winding coastal mountain road, steep cliff, ocean on the left），以及光线与氛围。最后一句写清这是一段几秒钟的镜头，例如：'4 second shot, vertical 9:16, 24fps, cinematic, highly detailed.'"
}}
